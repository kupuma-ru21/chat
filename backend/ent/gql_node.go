...
func (c *Client) noder(ctx context.Context, table string, id uuid.UUID) (Noder, error) {
	switch table {
	case date_message.Table:
		query := c.Date_Message.Query().
			Where(date_message.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, date_messageImplementors...); err != nil {
				return nil, err
			}
		}
...
func (c *Client) noders(ctx context.Context, table string, ids []uuid.UUID) ([]Noder, error) {
	...
	case date_message.Table:
		query := c.Date_Message.Query().
			Where(date_message.IDIn(ids...))
		query, err := query.CollectFields(ctx, date_messageImplementors...)
		if err != nil {
			return nil, err
		}
